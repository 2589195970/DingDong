...
public class AgentManagementServiceImpl implements AgentManagementService {
     ...
     * @throws BizException
     */
    public PageResult<AgentAccountListVO> selectAgentAccountListPage(AgentAccountSelectBO agentAccountSelectBO)
            throws BizException {
        // 获取当前登录用户的代理商编码
        ...
        if (totalRows == null || totalRows == BaseConstant.ZERO_INT) {
            return PageResultFactory.createPageResult(new ArrayList<>(), 0L, agentAccountSelectBO.getPageSize(),
                    agentAccountSelectBO.getPageNo());
        }
        ...
        return PageResultFactory.createPageResult(agentAccountListVOS, Long.valueOf(totalRows),
                agentAccountSelectBO.getPageSize(), agentAccountSelectBO.getPageNo());
    }
    ...
    public void updateBalance(AgentUpdateBalanceBO agentUpdateEnabledBO) throws BizException {
        ...
        withdrawalRecordService.insertAdminBalanceWithdrawal(agentAccount.getAgentCode(),
                agentUpdateEnabledBO.getBalanceYun(), agentUpdateEnabledBO.getType(), agentAccount);
    }
    ...
    public void deleteAgentAccount(Integer agentAccountId) throws BizException {
        ...
        // 删除该代理商名下的所有代理商品记录
        agentProductService.remove(new LambdaQueryWrapper<AgentProduct>()
     ...
     */
    @Override
    public PageResult<AgentRankingVO> selectAgentRankingPage(AgentRankingSelectBO agentRankingSelectBO)
            throws BizException {
        try {
            // 根据排名类型计算时间范围
            Long startTime = null;
            ...
            if (agentRankingSelectBO.getRankingType() != null && agentRankingSelectBO.getRankingType() != 0) {
                ...
                switch (agentRankingSelectBO.getRankingType()) {
                    case 1: // 本月排名
                        ...
                        end = now.withDayOfMonth(now.toLocalDate().lengthOfMonth()).withHour(23).withMinute(59)
                                .withSecond(59).withNano(999_000_000);
                        break;
                    case 2: // 昨日排名
