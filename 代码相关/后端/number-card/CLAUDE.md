...
> 所有我的问题,你需要判断难度来决定思考深度 ,例如复杂的问题你应该进入深度思考模式 而不是让我手动指定 /think 标签让你进入思考模式
...
Phase 3 - 基于官方格式自动生成Subagent文件 (真实创建):
  ...
  文件创建机制:
    ...
    6. SVN版本控 新创建的文件必须执行 Add to VCS 操作以纳入版本管理
...
**官方最佳实践**: For maximum efficiency, whenever you need to perform multiple independent operations, invoke all
...
This is a RuoYi-based Spring Boot application (v3.8.8) for managing number cards/virtual products. The system appears to be a business platform for managing agents, orders, products, and commission systems.

## Module Architecture

### Core Modules
- **ruoyi-admin**: Main web entry point (artifact: `number-card`)
- **ruoyi-framework**: Core framework with security, configuration
- **ruoyi-system**: System management (users, roles, menus, etc.)
- **ruoyi-common**: Common utilities, constants, APIs
- **ruoyi-quartz**: Scheduled tasks
- **ruoyi-generator**: Code generation

### Business Modules
- **mc-console**: Core business console module for number card management
- **mc-order**: Order processing module with RocketMQ integration
...
java -jar -Xms256m -Xmx1024m -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=512m ruoyi-admin/target/number-card.jar
```
...
- **MyBatis Plus**: 3.5.7
- **MySQL**: Database
- **Druid**: Connection pool (1.2.23)
- **Redis**: Caching

### Additional Libraries
- **FastJSON**: 2.0.43 (JSON processing)
- **JWT**: 0.9.1 (Authentication tokens)
- **Hutool**: 5.8.16 (Utility library)
- **OkHttp**: 4.9.0 (HTTP client)
- **RocketMQ**: 1.8.8.4.Final (Message queue in mc-order)
- **ZXing**: 3.3.0 (QR code generation)
...
- **Security**: JWT-based authentication (30min default expiry, configurable)
- **File Upload**: 10MB max file size, 20MB max request
- **Server**: Port 8080, Tomcat with 800 max threads

## Business Domain

### Core Entities (from service/mapper analysis)
- **Agent**: Agent management system
- **Product**: Virtual product catalog
- **Order**: Order processing and tracking
- **Commission**: Agent commission calculations
- **Withdrawal**: Agent withdrawal management
- **Number Status**: Number card status tracking
- **Upstream**: Third-party provider integration

### Agent Management
- Agent accounts, products, commissions
- Withdrawal applications and records
- Name audit system
- Extended URL management
...
Available API integrations in `ruoyi-common/src/main/java/com/ruoyi/common/apis/`:
- **DouYin**: E-commerce platform integration
- **GTH/WJK/YQE**: Various service providers
- **ECC/XuanKa**: Number card providers

## Database

### MyBatis Plus Configuration
- Entity scanning: `com.ruoyi.**.domain`
- Mapper locations: `classpath*:mapper/**/*Mapper.xml`
- Custom type handler: `AESEncipherTypeHandler` for encryption

## Development Guidelines

### Code Structure
- Controllers in: `ruoyi-admin/src/main/java/com/ruoyi/web/controller/`
- Business services in: `mc-console/src/main/java/com/ruoyi/console/service/`
- Data access in: `mc-console/src/main/java/com/ruoyi/console/mapper/`

### Security
- Spring Security + JWT authentication
- XSS protection enabled
- Rate limiting annotation available (`@RateLimiter`)
- Data scope annotation for permissions (`@DataScope`)

### API Documentation
- Swagger enabled at `/dev-api` path
- Access via: `http://localhost:8080/dev-api/swagger-ui/`

### Logging
- Logback configuration in `ruoyi-admin/src/main/resources/logback.xml`
- Debug level for `com.ruoyi` packages

## File Locations

### Key Files
- Main application: `ruoyi-admin/src/main/java/com/ruoyi/RuoYiApplication.java`
- Security config: Framework module
- Database config: Druid auto-configuration
