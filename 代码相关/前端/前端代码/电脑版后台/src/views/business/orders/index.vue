    ...
    <el-table ref="tables" v-loading="loading" :data="list" row-key="operatorReportId" border lazy
      ...
      <el-table-column label="订单状态" align="left" prop="companySimpleName" :show-overflow-tooltip="true">
        <template slot-scope="scope">
          ...
          <span
            v-if="scope.row.photoStatus !== undefined && scope.row.photoStatus !== null && scope.row.photoStatus !== 0">
                        照片审核：
      ...
      <el-table-column align="center" label="订单操作" width="150" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button @click="handleCommission(scope.row)" type="text" size="small"
                     v-hasPermi="['channel:channelManagement:edit']">重新下单
          </el-button>
          <br>
          <el-button @click="selectOrderBalanceClick(scope.row)" type="text" size="small"
                     v-hasPermi="['channel:channelManagement:edit']">查询余额
          </el-button>
          <br>
          <el-button @click="registroOP(scope.row)" type="text" size="small"
                     v-hasPermi="['channel:channelManagement:remove']">日志
          </el-button>
          <br>
          <el-button @click="handleDelete(scope.row)" type="text" size="small"
                     v-hasPermi="['channel:channelManagement:remove']">更改状态
          </el-button>
        </template>
      </el-table-column>
      <el-table-column align="center" label="照片操作" width="150" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <template
            v-if="scope.row.photoRequired !== undefined && scope.row.photoRequired !== null && scope.row.photoRequired == 1">
            <div>
              ...
              <el-button @click="handleCopyEditLink(scope.row)" type="text" size="small">复制证件上传链接</el-button>
              <br>
            </div>
    ...
    <el-dialog :title="photoUpload.title" :visible.sync="photoUpload.open" width="600px" append-to-body>
      <el-form ref="photoUploadForm" :model="photoUpload.form" :rules="photoUpload.rules" label-width="120px">
        ...
        <el-form-item
          ...
          <el-input v-model="photoUpload.form.idCardFrontUrl"
                    :placeholder="'请输入' + getPhotoFieldTitle('idCardFrontUrl')">
          ...
          <div v-if="photoUpload.form.idCardFrontUrl" style="margin-top: 10px;">
            <el-image :src="photoUpload.form.idCardFrontUrl" style="width: 160px; height: 100px;"
                      :preview-src-list="[photoUpload.form.idCardFrontUrl]"></el-image>
          </div>
        </el-form-item>
        <el-form-item
          ...
          <el-input v-model="photoUpload.form.idCardBackUrl"
                    :placeholder="'请输入' + getPhotoFieldTitle('idCardBackUrl')">
          ...
          <div v-if="photoUpload.form.idCardBackUrl" style="margin-top: 10px;">
            <el-image :src="photoUpload.form.idCardBackUrl" style="width: 160px; height: 100px;"
                      :preview-src-list="[photoUpload.form.idCardBackUrl]"></el-image>
          </div>
        </el-form-item>
        <el-form-item
          ...
          <el-input v-model="photoUpload.form.personPhotoUrl"
                    :placeholder="'请输入' + getPhotoFieldTitle('personPhotoUrl')">
          ...
          <div v-if="photoUpload.form.personPhotoUrl" style="margin-top: 10px;">
            <el-image :src="photoUpload.form.personPhotoUrl" style="width: 160px; height: 100px;"
                      :preview-src-list="[photoUpload.form.personPhotoUrl]"></el-image>
          </div>
        </el-form-item>
        <el-form-item
          ...
          <el-input v-model="photoUpload.form.customPhotoUrl"
                    :placeholder="'请输入' + getPhotoFieldTitle('customPhotoUrl') + (isPhotoFieldRequired(photoUpload.currentRow, 'customPhotoUrl') ? '' : '（选填）')">
          ...
          <div v-if="photoUpload.form.customPhotoUrl" style="margin-top: 10px;">
            <el-image :src="photoUpload.form.customPhotoUrl" style="width: 160px; height: 100px;"
                      :preview-src-list="[photoUpload.form.customPhotoUrl]"></el-image>
          </div>
        </el-form-item>
    ...
    <el-dialog :title="photoAudit.title" :visible.sync="photoAudit.open" width="1240px" append-to-body>
      <el-form ref="photoAuditForm" :model="photoAudit.form" :rules="photoAudit.rules" label-width="120px">
          ...
          <div class="photo-view-container">
            <div v-if="shouldShowPhotoField(photoAudit.currentRow, 'idCardFrontUrl') && photoAudit.form.idCardFrontUrl"
              ...
              <div class="photo-image-wrapper">
                <el-image :src="photoAudit.form.idCardFrontUrl" class="photo-image-audit"
                          :preview-src-list="[photoAudit.form.idCardFrontUrl]"></el-image>
              </div>
            ...
            <div v-if="shouldShowPhotoField(photoAudit.currentRow, 'idCardBackUrl') && photoAudit.form.idCardBackUrl"
                 class="photo-item">
              <div class="photo-image-wrapper">
                <el-image :src="photoAudit.form.idCardBackUrl" class="photo-image-audit"
                          :preview-src-list="[photoAudit.form.idCardBackUrl]"></el-image>
              </div>
              <div class="photo-info-card">
            ...
            <div v-if="shouldShowPhotoField(photoAudit.currentRow, 'personPhotoUrl') && photoAudit.form.personPhotoUrl"
                 class="photo-item">
              <div class="photo-image-wrapper">
                <el-image :src="photoAudit.form.personPhotoUrl" class="photo-image-audit"
                          :preview-src-list="[photoAudit.form.personPhotoUrl]"></el-image>
              </div>
              <div class="photo-info-card">
            ...
            <div v-if="shouldShowPhotoField(photoAudit.currentRow, 'customPhotoUrl') && photoAudit.form.customPhotoUrl"
                 class="photo-item">
              <div class="photo-image-wrapper">
                <el-image :src="photoAudit.form.customPhotoUrl" class="photo-image-audit"
                          :preview-src-list="[photoAudit.form.customPhotoUrl]"></el-image>
              </div>
              <div class="photo-info-card">
        ...
        <el-form-item label="审核备注" prop="auditRemark">
          <el-input v-model="photoAudit.form.auditRemark" type="textarea" :rows="4"
    ...
    <el-dialog :title="photoView.title" :visible.sync="photoView.open" width="1024px" append-to-body>
      ...
      <div class="photo-view-container">
        <div v-if="photoView.data.idCardFrontUrl && shouldShowPhotoField(photoView.data, 'idCardFrontUrl')"
             class="photo-item">
          <div class="photo-image-wrapper">
            <el-image :src="photoView.data.idCardFrontUrl" class="photo-image"
                      :preview-src-list="getPhotoPreviewList()"></el-image>
          </div>
          <div class="photo-info-card">
            <div class="photo-title">{{ getPhotoFieldTitle('idCardFrontUrl') }}</div>
          </div>
        ...
        <div v-if="photoView.data.idCardBackUrl && shouldShowPhotoField(photoView.data, 'idCardBackUrl')"
             class="photo-item">
          <div class="photo-image-wrapper">
            <el-image :src="photoView.data.idCardBackUrl" class="photo-image"
                      :preview-src-list="getPhotoPreviewList()"></el-image>
          </div>
          <div class="photo-info-card">
            <div class="photo-title">{{ getPhotoFieldTitle('idCardBackUrl') }}</div>
        ...
        <div v-if="photoView.data.personPhotoUrl && shouldShowPhotoField(photoView.data, 'personPhotoUrl')"
             class="photo-item">
          <div class="photo-image-wrapper">
            <el-image :src="photoView.data.personPhotoUrl" class="photo-image"
                      :preview-src-list="getPhotoPreviewList()"></el-image>
          </div>
          <div class="photo-info-card">
            <div class="photo-title">{{ getPhotoFieldTitle('personPhotoUrl') }}</div>
        ...
        <div v-if="photoView.data.customPhotoUrl && shouldShowPhotoField(photoView.data, 'customPhotoUrl')"
             class="photo-item">
          <div class="photo-image-wrapper">
            <el-image :src="photoView.data.customPhotoUrl" class="photo-image"
                      :preview-src-list="getPhotoPreviewList()"></el-image>
          </div>
          <div class="photo-info-card">
            <div class="photo-title">{{ getPhotoFieldTitle('customPhotoUrl') }}</div>
  ...
  methods: {
    ...
    handleImageUploadSuccess(response) {
      if (response.code === 200) {
        const url = response.url || response.fileName || (response.data && response.data.url);
