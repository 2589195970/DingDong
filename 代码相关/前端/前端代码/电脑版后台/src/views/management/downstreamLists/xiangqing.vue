<template>
    <div class="app-container">
        ...
        <el-table ref="tables" v-loading="loading" :data="list" row-key="operatorReportId" border lazy
            ...
            <el-table-column label="提现类型" align="center" prop="withdrawalType" :show-overflow-tooltip="true">
                <template slot-scope="scope">
            ...
            <el-table-column label="提现金额" align="center" prop="withdrawalAmount">
                <template slot-scope="scope">
                    <p>{{scope.row.withdrawalAmount * 0.01}}元</p>
                </template>
            </el-table-column>
            <el-table-column label="手续费" align="center" prop="withdrawalRateAmount" width="110"
                :show-overflow-tooltip="true" >
                <template slot-scope="scope">
                    <p>{{(scope.row.withdrawalRateAmount * 0.01).toFixed(2)}}元</p>
                </template>
            </el-table-column>
...
<script>
    import { selectWithdrawalApplicationListPage, updateWithdrawalApplication, deletelithdrawalApplication,selectWithdrawalRecordDetailsListPage} from "@/api/monitor/finance";
    export default {
        name: "Operlog",
        ...
        methods: {
            ...
            getList() {
                this.queryParams.agentCode = this.$route.params.agentCode;
                selectWithdrawalRecordDetailsListPage(this.queryParams).then((res) => {
                    if (res.data.rows) {
